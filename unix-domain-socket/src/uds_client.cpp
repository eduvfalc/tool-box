...
UDSClient::UDSClient() : m_buffer(BUF_SIZE)  {
...
UDSClient::Send() {
    ssize_t num_read;
    while ((num_read = read(STDIN_FILENO, m_buffer.data(), m_buffer.size())) > 0)
        if (write(m_server_file_descriptor, m_buffer.data(), num_read) != num_read)
